...
#defiine FILE_NAME_MAX_LENGTH 256
...
char *get_file_case_insensitive(const char *file_name) {
    if (file_name == NULL || strlen(file_name) == 0 || strlen(file_name) > FILE_NAME_MAX_LENGTH) {
        return NULL; // Invalid file name
    }

    // Check for special characters or sequences
...
int main(int argc, char *argv[]) {
    ...
    // enable SO_REUSEADDR // CHECK THIS
    int opt = 1;
